import { MessageCircle, Star, Award, Shield, ArrowRight, Users, Sparkles, MessageSquare } from 'lucide-react';
...
export default function HomePage({ onNavigate }: HomePageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
          ...
          <div className="grid grid-cols-2 gap-16 items-center max-w-7xl mx-auto min-h-[calc(100vh-5rem)] py-12">
            <div className="space-y-8 z-10">
              <div className="inline-flex items-center space-x-2 bg-cyan-500/10 border border-cyan-400/30 rounded-full px-5 py-2.5">
                ...
                <span className="text-xs font-bold text-cyan-300 tracking-wider">20+ YEARS EXCELLENCE</span>
              </div>
              ...
              <p className="text-xl text-slate-300 leading-relaxed max-w-lg">
                Professional sound systems engineered for perfection. From home theaters to concert venues.
              </p>
      ...
      {/* Mobile Hero */}
      <section className="lg:hidden relative text-white overflow-hidden min-h-screen">
        <div
          className="absolute inset-0 bg-cover bg-center"
        ...
        <div className="container mx-auto px-4 py-16 relative">
          <div className="space-y-8 text-center">
            ...
            <h1 className="text-4xl sm:text-5xl font-black leading-tight">
              Premium Audio
              <br />
              <span className="bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent">That Delivers</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={() => onNavigate('products')}
                className="group bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-10 py-4 rounded-xl font-bold hover:shadow-2xl hover:shadow-cyan-500/40 transition-all duration-300 flex items-center justify-center space-x-2"
              >
                <span>View Products</span>
                <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform" />
              </button>
              <button
                onClick={() => onNavigate('contact')}
                className="bg-white/10 border-2 border-white/20 text-white px-10 py-4 rounded-xl font-bold hover:bg-white/20 hover:border-white/30 transition-all duration-300"
              >
                Contact Us
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 pt-6 max-w-md mx-auto">
              {[
                ...
                <div
                  key={idx}
                  className="text-center p-5 bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm"
                >
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {testimonialsData.map((testimonial, idx) => (
              <div
                key={testimonial.id}
                className={`bg-white/5 backdrop-blur-sm rounded-2xl lg:rounded-3xl p-6 lg:p-8 hover:bg-white/10 hover:scale-105 hover:-translate-y-1 transition-all duration-300 border-2 border-white/10 hover:border-cyan-400/30 text-center animate-fade-in delay-${(idx % 4) * 100 + 100}`}
              >
                <div className="flex items-center justify-center mb-4 space-x-1">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="h-4 lg:h-5 w-4 lg:w-5 fill-yellow-400 text-yellow-400 hover:scale-125 transition-transform" />
                  ))}
                </div>
